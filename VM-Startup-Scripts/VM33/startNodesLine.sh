...
$CCNL_HOME/bin/ccn-lite-relay -v trace -u 9006 -x /tmp/mgmt-nfn-relay-f.sock &
sleep 0.1
...
#----------------------------|
#9006 -> 9001
	FACEID=`$CCNL_HOME/bin/ccn-lite-ctrl -x /tmp/mgmt-nfn-relay-f.sock newUDPface any ${VMS[4]} 9005 \
...
#9006 -> 9007
	FACEID=`$CCNL_HOME/bin/ccn-lite-ctrl -x /tmp/mgmt-nfn-relay-f.sock newUDPface any ${VMS[6]} 9007 \
	  | $CCNL_HOME/bin/ccn-lite-ccnb2xml | grep FACEID | sed -e 's/^[^0-9]*\([0-9]\+\).*/\1/'`
	sleep 0.1
