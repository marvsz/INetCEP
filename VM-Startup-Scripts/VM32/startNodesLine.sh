...
$CCNL_HOME/bin/ccn-lite-relay -v trace -u 9005 -x /tmp/mgmt-nfn-relay-e.sock &
sleep 0.1
...
#----------------------------|
#9005 -> 9001
	FACEID=`$CCNL_HOME/bin/ccn-lite-ctrl -x /tmp/mgmt-nfn-relay-e.sock newUDPface any ${VMS[3]} 9004 \
...
#9005 -> 9006
	FACEID=`$CCNL_HOME/bin/ccn-lite-ctrl -x /tmp/mgmt-nfn-relay-e.sock newUDPface any ${VMS[5]} 9006 \
	  | $CCNL_HOME/bin/ccn-lite-ccnb2xml | grep FACEID | sed -e 's/^[^0-9]*\([0-9]\+\).*/\1/'`
	sleep 0.1
