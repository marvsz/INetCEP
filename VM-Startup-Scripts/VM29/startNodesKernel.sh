...
#9002 -> 9004
	FACEID=`$CCNL_HOME/bin/ccn-lite-ctrl -u 127.0.0.1/9002 newUDPface any ${VMS[3]} 9004 \
	  | $CCNL_HOME/bin/ccn-lite-ccnb2xml | grep FACEID | sed -e 's/^[^0-9]*\([0-9]\+\).*/\1/'`
	sleep 0.1
...
#9002 -> 9005
	FACEID=`$CCNL_HOME/bin/ccn-lite-ctrl -u 127.0.0.1/9002 newUDPface any ${VMS[4]} 9005 \
  	| $CCNL_HOME/bin/ccn-lite-ccnb2xml | grep FACEID | sed -e 's/^[^0-9]*\([0-9]\+\).*/\1/'`
	sleep 0.1
