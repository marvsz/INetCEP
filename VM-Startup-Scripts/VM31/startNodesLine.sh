...
$CCNL_HOME/bin/ccn-lite-relay -v trace -u 9004 -x /tmp/mgmt-nfn-relay-d.sock &
sleep 0.1
...
#----------------------------|
#9004 -> 9001
	FACEID=`$CCNL_HOME/bin/ccn-lite-ctrl -x /tmp/mgmt-nfn-relay-d.sock newUDPface any ${VMS[2]} 9003 \
...
#9004 -> 9005
	FACEID=`$CCNL_HOME/bin/ccn-lite-ctrl -x /tmp/mgmt-nfn-relay-d.sock newUDPface any ${VMS[4]} 9005 \
	  | $CCNL_HOME/bin/ccn-lite-ccnb2xml | grep FACEID | sed -e 's/^[^0-9]*\([0-9]\+\).*/\1/'`
	sleep 0.1
