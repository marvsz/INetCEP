...
nohup bash startNode.sh 9001 /tmp/mgmt-nfn-relay-a.sock > nodeA.log &
sleep 0.1
nohup bash startNode.sh 9002 /tmp/mgmt-nfn-relay-b.sock > nodeB.log &
sleep 2s
...
~/INetCEP/ccn-lite/bin/ccn-lite-ctrl -x /tmp/mgmt-nfn-relay-b.sock prefixreg /nodeA $FACEID ndn2013 \
  | ~/INetCEP/ccn-lite/bin/ccn-lite-ccnb2xml
sleep 1s
nohup bash startCS.sh a 9001 9011 > CSA.log &
sleep 0.1
nohup bash startCS.sh a 9002 9012 > CSB.log &
sleep 6s
