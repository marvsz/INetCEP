...
struct ccnl_pkt_s *createNewPacket(struct ccnl_prefix_s *prefix, char *buf, int len) {
    ...
    for (it = 0; it < numPackets; ++it) {
        unsigned char *buf2;
        int packetsize = size / 2, len4 = 0, len5, hdrlen = 0;
        unsigned char *packet = (unsigned char *) ccnl_malloc(sizeof(char) * packetsize * 2);
        hdrlen += ccnl_ccnb_mkHeader(packet + len4, CCNL_DTAG_FRAG, CCN_TT_DTAG);
        len4 += hdrlen;
        //if(it == numPackets -1){
        ...
        pkt->content = pkt->buf->data + contentpos + hdrlen;
        pkt->contlen = len;
        ccnl_free(buf2);
        ccnl_free(packet);
    }
...
void ccnl_makeQueryPersistent(struct ccnl_interest_s *intr, struct ccnl_relay_s *ccnl, struct ccnl_prefix_s *pfx) {
    ...
    for (i = ccnl->pit; i; i = i->next) {
        ...
        if (!ccnl_prefix_cmp(i->pkt->pfx, NULL, pfx, CMP_EXACT)) {
            ...
            DEBUGMSG(DEBUG, "Found already pending persistent Interest\n");
            break;
        } else {
    ...
    if (!i) {
        i = ccnl_mkPersistentInterestObject(ccnl_prefix_dup(pfx), &int_opts);
...
#endif
    {
        uint64_t timeDifference = tend.tv_nsec -
                                  tstart.tv_nsec;//((uint64_t)tend.tv_sec + 1.0e-9*tend.tv_nsec) - ((uint64_t)tstart.tv_sec + 1.0e-9*tstart.tv_nsec);

#ifndef CCNL_LINUXKERNEL
        DEBUGMSG(EVAL, "Builtin Add took about %lu nano seconds\n", timeDifference);

#else
        DEBUGMSG(EVAL,"Builtin Kernel Add took about %llu nano seconds\n",timeDifference);
#endif
    }
...
op_builtin_window(struct ccnl_relay_s *ccnl, struct configuration_s *config,
    ...
    int prefixLength = strlen((s = ccnl_prefix_to_path(tupleprefix))) + strlen("state") + get_int_len(quantity) +
                       get_int_len(unit) + 3;
#ifndef CCNL_LINUXKERNEL
    ccnl_free(s);
#endif
    char statePrefix[prefixLength];
    sprintf(statePrefix, "state%s/%i/%i", (s = ccnl_prefix_to_path(tupleprefix)), quantity, unit);
#ifndef CCNL_LINUXKERNEL
    ccnl_free(s);
#endif
    ...
    char stateContent[stateContLen + 1];
    if (state) {
    ...
    char endResult[tupleContLen + 2 + stateContLen + 1];
    ...
    DEBUGMSG(INFO, "WINDOW: result was found ---> handle it (%s), prog=%s, pending=%s\n",
             (s = ccnl_prefix_to_path(stateprefix)), prog, pending);
#ifndef CCNL_LINUXKERNEL
    ccnl_free(s);
#endif
...
#ifndef CCNL_LINUXKERNEL
    DEBUGMSG(EVAL, "Builtin Window took about %lu nano seconds\n", timeDifference);

#else
    DEBUGMSG(EVAL,"Builtin Kernel Window took about %llu nano seconds\n",timeDifference);
#endif
    return cp;
}
