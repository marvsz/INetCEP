...
main(int argc, char **argv)
{
    ...
    while ((opt = getopt(argc, argv, "hc:d:e:g:i:o:p:s:t:u:6:v:w:x:")) != -1) {
        switch (opt) {
        case 'c':
            max_cache_entries = strtol(optarg, (char**) NULL,10);
            break;
        case 'd':
            datadir = optarg;
            break;
        case 'e':
            ...
            break;
        case 'g':
            inter_pkt_interval = strtol(optarg, (char**) NULL,10);
            break;
        case 'i':
            inter_ccn_interval = strtol(optarg, (char**) NULL,10);
            break;
#ifdef USE_ECHO
        case 'o':
            echopfx = optarg;
        ...
        case 's':
            ...
            break;
        case 't':
            httpport = strtol(optarg, (char**) NULL,10);
            break;
        case 'u':
            if (udpport1 == -1)
                udpport1 = strtol(optarg, (char**) NULL,10);
            else
                udpport2 = strtol(optarg, (char**) NULL,10);
            break;
        case '6':
            if (udp6port1 == -1)
                udp6port1 = strtol(optarg, (char**) NULL,10);
            else
                udp6port2 = strtol(optarg, (char**) NULL,10);
            break;
        case 'v':
#ifdef USE_LOGGING
            if (isdigit(optarg[0]))
