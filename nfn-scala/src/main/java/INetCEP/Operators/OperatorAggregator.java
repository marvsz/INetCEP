...
public class OperatorAggregator extends OperatorA {
    ...
    public String genNFNQuery(String communicationAppraoch) {
        ...
        switch (communicationAppraoch.toLowerCase()) {
            case "pra":
                nfn = new NFNQueryCreator("(call " + (this.parameters.length + 3) + " /node/nodeQuery/nfn_service_Aggregator");
                int counter = 1;
                ...
                break;
            case "ucl":
                nfn = new NFNQueryCreator("(call " + (this.parameters.length + 2) + " /node/nodeQuery/nfn_service_Aggregator");
