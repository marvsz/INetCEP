...
import nfn.service.PlacementServices.{DecentralizedPRAPlacement, InitiativePRAPlacement, LocalPRAPlacement}
    ...
    if (currentNode._Vprocessed) {
      ...
      if (currentNode.right != null && !currentNode.right._Vprocessed) {
        ...
        val query = currentNode._type match {
          ...
          case Operator.AGGREGATION => name.replace("[Q1]",currentNode.left._value)
          case Operator.SEQUENCE => name.replace("[Q1]",currentNode.left._value)
...
final case class NoSuchPRAPlacementException(private val message: String = "", private val cause: Throwable = None.orNull) extends Exception(message, cause)

object PRAPlacement {

  def apply(s: String,_nodeName:String, _mapping: NodeMapping, _ccnApi: ActorRef, _root: Map, _paths: ListBuffer[Paths], _maxPath: Int, _evalHandler: EvaluationHandler, _opCount: Int, _communicationApproach:String): PRAPlacement = {
    s match {
      case "centralized" =>  new InitiativePRAPlacement(_nodeName:String, _mapping: NodeMapping, _ccnApi: ActorRef, _root: Map, _paths: ListBuffer[Paths], _maxPath: Int, _evalHandler: EvaluationHandler, _opCount: Int)
      case "decentraliized" => new DecentralizedPRAPlacement(_nodeName:String, _mapping: NodeMapping, _ccnApi: ActorRef, _root: Map, _paths: ListBuffer[Paths], _maxPath: Int, _evalHandler: EvaluationHandler, _opCount: Int)
      case "local" => new LocalPRAPlacement(_nodeName:String, _mapping: NodeMapping, _ccnApi: ActorRef, _root: Map, _paths: ListBuffer[Paths], _maxPath: Int, _evalHandler: EvaluationHandler, _opCount: Int)
