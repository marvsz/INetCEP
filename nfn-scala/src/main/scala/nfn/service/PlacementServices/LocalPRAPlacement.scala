...
import nfn.tools.{EvaluationHandler, Helpers, Networking}
import scala.concurrent.duration._
import scala.collection.mutable.ListBuffer
import scala.language.postfixOps

class LocalPRAPlacement(_nodeName: String, _mapping: NodeMapping, _ccnApi: ActorRef, _root: Map, _paths: ListBuffer[Paths], _maxPath: Int, _evalHandler: EvaluationHandler, _opCount: Int) extends PRAPlacement {
    ...
    if (currentNode._Vprocessed) {
      ...
      if (currentNode.right != null && !currentNode.right._Vprocessed) {
        ...
        val query = currentNode._type match {
          ...
          case Operator.FILTER => name.replace("[Q1]", currentNode.left._value)//.replace("[Q1]",currentNode.left._query.replace("nodeQuery",currentNode.left._executionNode))
          case Operator.JOIN => name.replace("[Q1]", currentNode.left._value).replace("[Q2]", currentNode.right._value)//.replace("[Q1]",currentNode.left._query.replace("nodeQuery",currentNode.left._executionNode)).replace("[Q2]",currentNode.right._query.replace("nodeQuery",currentNode.right._executionNode))//.replace("[Q1]", currentNode.left._value).replace("[Q2]", currentNode.right._value)
          case Operator.AGGREGATION => name.replace("[Q1]", currentNode.left._value)//.replace("[Q1]",currentNode.left._query.replace("nodeQuery",currentNode.left._executionNode))
          case Operator.SEQUENCE => name.replace("[Q1]", currentNode.left._value)//.replace("[Q1]",currentNode.left._query.replace("nodeQuery",currentNode.left._executionNode))
          case Operator.PREDICT1 => name.replace("[Q1]", currentNode.left._value)//.replace("[Q1]",currentNode.left._query.replace("nodeQuery",currentNode.left._executionNode))//.replace("[Q1]",currentNode.left._value)
          case Operator.PREDICT2 => name.replace("[Q1]", currentNode.left._value)//.replace("[Q1]",currentNode.left._query.replace("nodeQuery",currentNode.left._executionNode))//.replace("[Q1]",currentNode.left._value)
          case Operator.HEATMAP => name.replace("[Q1]", currentNode.left._value)//.replace("[Q1]",currentNode.left._query.replace("nodeQuery",currentNode.left._executionNode))//.replace("[Q1]",currentNode.left._value)
          case _ => name
        }
        ...
        currentNode._value = Helpers.executeNFNQuery(currentNode._query,remoteNodeName,ccnApi,60)
        ...
        if (currentNode.parent == null) {
          LogMessage(nodeName,currentNode._value)
          currentNode
  ...
  override def process(): String = {
    ...
    val intermediateResult = Networking.fetchContentFromNetwork(output, ccnApi,500 milliseconds)
    if(intermediateResult.isDefined){
      output = new String(intermediateResult.get.data)
    }
    else{
      output = "Fail"
    }
    if (output != null && !output.isEmpty)
      ...
      output += "No Results!"
    LogMessage(nodeName, s"Query Execution Completed. output is $output")
    output
  }
