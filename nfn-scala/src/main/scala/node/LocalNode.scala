...
case class LocalNode(routerConfig: RouterConfig, maybeComputeNodeConfig: Option[ComputeNodeConfig]) extends LazyLogging {
  ...
  def sendReceive(req: Interest)(implicit useThunks: Boolean): Future[Content] = {
    (nfnMaster ? NFNApi.CCNSendReceive(req, useThunks)).mapTo[CCNPacket] map {
      ...
      case ci: PersistentInterest => throw new Exception("A constant interest was returned, this should never happen")
      case rci: RemovePersistentInterest => throw new Exception("A remove constant interest was returned, this should never happen")
