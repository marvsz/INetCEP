...
object Monitor {
  ...
  case class PersistentInterestInfoLog(`type`: String = "constantInterest", name: String) extends PacketInfoLog
  case class RemovePersistentInterestInfoLog(`type`: String = "removeConstantInterest", name: String) extends PacketInfoLog
...
case class Monitor() extends Actor {
  ...
  implicit val formats =
    DefaultFormats.withHints(
      ShortTypeHints(List(classOf[InterestInfoLog], classOf[PersistentInterestInfoLog], classOf[RemovePersistentInterestInfoLog], classOf[ContentInfoLog])) )
...
//      JObject(List(JField(type,JString(interest)), JField(name,JString(/docRepo1/docs/doc0/(@x call 2 /nfn_service_impl_WordCountService x )/NFN))))
      ...
      (parsedPacket \\ "type").extract[String] match {
        ...
        case t @ "constantInterest" => {
          PersistentInterestInfoLog(t, name)
        }
        case t @ "removeConstantInterest" => {
          RemovePersistentInterestInfoLog(t, name)
        }
        case t @ "content" =>
