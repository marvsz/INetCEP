...
case class OmnetIntegration(nodes: Set[NodeLog],
  ...
  def loggedPacketToType(lp: PacketInfoLog): String = lp match {
    ...
    case ci: PersistentInterestInfoLog => "constantInterest"
    case rci: RemovePersistentInterestInfoLog => "removeConstantInterest"
  ...
  def createTransmissionJson:String = {
    ...
    def jsonPacket(p: PacketInfoLog): JValue =
      p match {
        ...
        case ci: PersistentInterestInfoLog =>
          ("name" -> ci.name) ~ ("type" -> "constantInterest")
        case rci: RemovePersistentInterestInfoLog =>
